...
class TemplateLibrary:
    ...
    REWRITE_PROMPT = ChatPromptTemplate.from_messages([
        ("system",
         """
        你是一个查询优化专家。请根据对话历史，将用户的问题改写为一个独立、完整的查询。
    ...
    # Collection 路由提示 - 用于查询时选择 collection
    COLLECTION_ROUTER_PROMPT = ChatPromptTemplate.from_messages([
        ("system",
         """
        你是一个智能路由专家。根据用户的问题，判断应该从哪个数据集合中检索信息。
    ...
    # 文档分类提示 - 用于文档存储时选择 collection
    DOCUMENT_CLASSIFIER_PROMPT = ChatPromptTemplate.from_messages([
        ("system",
         """
        你是一个文档分类专家。分析文档内容，判断它应该存储到哪个数据集合。
